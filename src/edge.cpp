...
void drawArrowAt(QPainter *painter, QPointF at, double angle, double arrowSize) {
    ...
    QPointF destArrowP2 =
            at + QPointF(sin(angle - M_PI + M_PI / 3) * arrowSize, cos(angle - M_PI + M_PI / 3) * arrowSize);
...
void Edge::setBandwidth(double bandwidth) { this->bandwidth = bandwidth; }

void Edge::setEdgeType(EdgeType type) { this->edgeType = type; }

double Edge::getWeight() const { return this->weight; }

double Edge::getFlow() const { return this->flow; }

double Edge::getBandwidth() const { return this->bandwidth; }

EdgeType Edge::getEdgeType() { return this->edgeType; }
void Edge::adjust() {
    if (!source || !dest)
        return;
...
void Edge::setFlagsPtr(QFlags<GraphFlags> *flagsPtr) { graphFlags = flagsPtr; }
...
void Edge::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *) {
    ...
    if (this->edgeType == EdgeType::SingleDirection) {
        ...
        QPointF offsetPnt =
                QPointF(midPoint.x() + curvines * cos(teta + M_PI / 2), midPoint.y() + curvines * sin(teta + M_PI / 2));
        ...
        drawTextAt(painter,
                   QPointF(midPoint.x() + curvines / 2 * cos(teta + M_PI / 2),
                           midPoint.y() + curvines / 2 * sin(teta + M_PI / 2)),
                   text);
